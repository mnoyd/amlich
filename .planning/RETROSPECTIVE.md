...
### What Was Inefficient
- **No formal `v1.7-MILESTONE-AUDIT.md` was produced before milestone close.** Milestone closed directly after Phase 25 with no separate audit pass. Phase 25's baseline guards + E2E smoke served as partial validation, but no comprehensive audit re-derived satisfaction from VERIFICATION.md / SUMMARY frontmatter / REQUIREMENTS rollup across all 6 phases the way v1.4/v1.5/v1.6 audits did. **Backfilled as a retrospective audit on 2026-07-20** (1 day post-close) — produced `.planning/milestones/v1.7-MILESTONE-AUDIT.md` with status `tech_debt` (15/15 satisfied, 0 gaps). Retrospective audit confirmed the close was correct, but the missing pre-close audit still lost the forcing-function safety net for ~24 hours.
...
### Key Lessons
1. **A 5-day milestone with no pre-close audit is a calculated risk that did NOT pay off cleanly.** Phase 25's baseline guards caught the regression-class risks (dep-tree drift, golden-discipline weakening), but the cross-phase integration audit is a different failure mode. v1.5/v1.6 audits passed clean, but the value of running the audit is partly the act of running it — skipping it loses the forcing function. **Backfilled the audit 24h later as a retrospective; it confirmed the close was correct (15/15 satisfied, 0 gaps), but the workflow should require `/gsd-audit-milestone` BEFORE `/gsd-complete-milestone` for v1.8+.**
...
7. **Retrospective audits work but cost more than pre-close audits.** The 2026-07-20 retrospective v1.7 audit produced the same result as a pre-close audit would have (15/15 satisfied, 0 gaps, tech_debt status), but required rebuilding context across 14 SUMMARYs + 6 VERIFICATIONs + REQUIREMENTS archive after the milestone-close workflow had already collapsed everything into `<details>` + archives. Pre-close audits reuse the live REQUIREMENTS.md + ROADMAP.md for free.
...
| v1.7 | ~5 | 6 | Sibling-newtype over closed-enum extension, single-named-helper boundary gates (CRIT-2/4), per-step evidence envelope + composite (CRIT-6), runtime-built needle patterns for grep guards, parallel tracks with explicit merge points, **first milestone closed without a pre-close audit (backfilled 24h later as retrospective — v1.8+ should require audit-before-close)** |
...
10. **Run the audit BEFORE closing the milestone** (v1.7 lesson) — retrospective audits produce the same result but cost more (rebuilding context across collapsed/archived artifacts). For v1.8+: `/gsd-audit-milestone` should run BEFORE `/gsd-complete-milestone`. The forcing function matters even when the audit will pass clean.
