...
- [x] **INT-11**: User-of-semantic-graph can find Hexagram nodes (chủ quẻ + biến quẻ) wired via `LocatedAt`/`Transforms` edges and a composite cross-link fact node, emitted by additive `add_iching_facts()` + `add_direction_composite_facts()` builder methods (v1.5 FlyingStar/Offering precedent).
- [x] **INT-12**: User-of-`DaySnapshot` can find additive `iching_cast: Option<IChingCastSummary>` AND `direction_cross_link: Option<DirectionCrossLinkSummary>` fields (`#[serde(default, skip_serializing_if = "Option::is_none")]`), with a v1.6→v1.7 backward-compat round-trip test proving a v1.6 producer JSON deserializes cleanly and re-serializes without unexpected fields. **Closed across 24-01 + 23-03 + 24-03**: `iching_cast` field + `enrich_day_snapshot_with_iching` helper + 18-test black-box integration suite (24-01) + `direction_cross_link` field + immutable `enrich_day_snapshot_with_direction_cross_link` helper (Phase 23-03, shipped before Plan 24-02 executed) + combined-strip v1.6→v1.7 round-trip tests in `tests/day_snapshot_v14_compat.rs` (Plan 24-03: Test 10 strips BOTH v1.7 fields together with semantic-equality + no `null` + no new keys; Test 11 byte-equal round-trip with BOTH fields populated including CRIT-6 evidence contract + cross-link shape; Test 12 `skip_serializing_if` honoured for both fields simultaneously).
- [x] **INT-13**: User-of-validation can find ≥10 IChing golden casting cases cross-checked against ≥2 independent sources (divergences logged as `KnownDivergence`, not silently corrected) plus a 2026 E2E smoke extending `integration_2026_smoke.rs`, and the full crate test suite green with zero regressions.
...
| ICH-05 | 24 | Complete (24-01 PLAN: IChingEvaluator + IChingQuery + IChingEvaluation + IChingCastSummary + HexagramEntryProjection + COMPOSITE_ICHING_CONSULTATION named const; locked 4-envelope evidence vector (3 primitives with distinct source_ids including `mai-hoa-dich-so` and `kinh-dich` + 1 composite `rule.composite.iching_consultation` in Derived family); Tier-0 ActionEvaluator adapter returns Ok(empty ActionEvaluation) ignoring personal_input (MOD-7); IChingQuery::from_snapshot / from_lunar_inputs with NFC normalisation + chi_hour_index 0..=11 validation; 19 inline + 18 black-box integration tests green; CRIT-3 + WASM-safety + source_id_guard grep guards clean; zero regressions vs Phase 23-03's 1062-test baseline; 1101 passing tests across 48 test groups in full crate) |
...
| INT-11 | 24 | Complete |
| INT-12 | 24 | Complete (24-01 PLAN: iching_cast additive field + enrich_day_snapshot_with_iching helper + 18-test black-box integration suite; Phase 23-03 shipped direction_cross_link additive field + enrich_day_snapshot_with_direction_cross_link helper before Plan 24-02 executed; 24-03 PLAN: 3 combined-strip v1.6→v1.7 round-trip tests in tests/day_snapshot_v14_compat.rs — Test 10 BLOCKER 5 FIX strips both v1.7 fields together + semantic-equality + no null + no new keys, Test 11 byte-equal round-trip with BOTH fields populated + CRIT-6 evidence contract + cross-link shape, Test 12 skip_serializing_if honoured for both fields simultaneously; 12/12 day_snapshot_v14_compat tests green; full crate 1117 tests green with zero regressions vs Plan 24-02's 1114-test baseline) |
| INT-13 | 25 | Complete (25-01 PLAN: `e2e_2026_smoke_v17_iching_and_cross_link_wiring_on_representative_dates` in `integration_2026_smoke.rs` exercises ALL FIVE v1.7 surfaces — Phase 22 casting chain + Phase 24-01 immutable IChing enrichment with CRIT-6 4-envelope evidence contract + Phase 23-03 immutable direction cross-link enrichment with 8-cell + dual-source provenance + Phase 24-02 semantic-graph wiring — together on Tết 2026 + 4 Sóc dates spanning distinct lunar months; `cargo_dependency_tree_unchanged_from_v16` (SC4) + `int13_golden_dataset_cross_source_discipline_holds` (SC1) in new `tests/v17_baseline_guards.rs` lock the v1.6 dep-tree shape + INT-13 golden discipline as runtime invariants; full crate 1120 tests green with zero regressions vs Plan 24-03's 1117-test baseline) |
