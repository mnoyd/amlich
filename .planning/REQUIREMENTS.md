...
- [ ] **INT-12**: User-of-`DaySnapshot` can find additive `iching_cast: Option<IChingCastSummary>` AND `direction_cross_link: Option<DirectionCrossLinkSummary>` fields (`#[serde(default, skip_serializing_if = "Option::is_none")]`), with a v1.6→v1.7 backward-compat round-trip test proving a v1.6 producer JSON deserializes cleanly and re-serializes without unexpected fields. **Partial close in 24-01**: `iching_cast` field + `enrich_day_snapshot_with_iching` helper + 17-test black-box integration suite (byte-equal round-trip with v1.6 fields intact). `direction_cross_link` + combined-strip v1.6→v1.7 round-trip test ship in Plan 24-03 (cross-link half is already implemented in Phase 23-03; 24-03 adds the combined-strip integration test that closes the requirement).
...
| INT-11 | 24 | Complete |
| INT-12 | 24 | Partial (24-01 PLAN: iching_cast additive field + enrich_day_snapshot_with_iching helper + 17-test black-box integration suite including byte-equal round-trip with v1.6 fields intact; direction_cross_link + combined-strip v1.6→v1.7 round-trip test ship in Plan 24-03) |
| INT-13 | 25 | Pending |
