...
| DEC-0029 | 2026-08-11 | v1.10 Tier-0 scope split: Thập nhị kinh nạp địa chi (十二經納地支) historical branch-channel association ships as `SOURCE_SHI_ER_JING_NA_DI_ZHI`; full Tý Ngọ Lưu Chú (`子午流注`) is reserved for a future separately reviewed milestone and `ty-ngo-luu-chu` must never be emitted. New `pub const` registered in `sources.rs`; second CI guard `ty_ngo_luu_chu_substring_never_appears_in_production_source` locks the substring out of `crates/amlich-core/src/` | [../../docs/adr/0003-separate-branch-channel-association-from-ty-ngo-luu-chu.md](../../docs/adr/0003-separate-branch-channel-association-from-ty-ngo-luu-chu.md) |
| DEC-0030 | 2026-08-11 | v1.10 module placement: `traditional_wellness` is a sibling of `reasoning/`, NOT nested under `almanac/`, because the surface does not contribute to Day Assessment / Hour Ranking / Direction Assessment per ADR-0003 consequences. Owns `disclaimer`, `divergence`, `branch_channel` submodules; corpus JSON lives under `data/traditional-wellness/` | [../../docs/adr/0003-separate-branch-channel-association-from-ty-ngo-luu-chu.md](../../docs/adr/0003-separate-branch-channel-association-from-ty-ngo-luu-chu.md) |
| DEC-0031 | 2026-08-16 | v1.10 Phase 02-01 seasonal provenance split: four-season cultivation paraphrases ship as `SOURCE_HUANGDI_NEIJING_SUWEN` (scope strictly `素問 四氣調神大論篇第二`); the astronomical solar-term engine keeps its existing attribution (`amlich-solar-term-engine` Snapshot envelope); the 24-term → 4-season join at the four Lập boundaries is a disclosed Amlich composition emitted as exactly one Derived envelope `rule.composite.seasonal_wellness` (LH-DIV-04). Frozen mapping lives in code (`season_for_term_index`), NOT in the corpus; corpus is exactly 4 rows | [../milestones/v1.10-phases/02-seasonal-cultivation-context/02-01-PLAN.md](../milestones/v1.10-phases/02-seasonal-cultivation-context/02-01-PLAN.md) |
| DEC-0032 | 2026-08-16 | v1.10 Phase 02-01 unsourced-copy replacement: the 24 per-term `health` lists in `data/tiet-khi.json` (both repo copies) are emptied — schema retained for additive serialization compatibility — so no surface (core/API/TUI/desktop) can render unsourced clinical-sounding seasonal health advice; the only shipped seasonal wellness content is the reviewed 4-profile Suwen corpus carrying disclaimer, review state, safety class, and LH-DIV-04/05/07 divergences | [../milestones/v1.10-phases/02-seasonal-cultivation-context/REVIEWER-PACK.md](../milestones/v1.10-phases/02-seasonal-cultivation-context/REVIEWER-PACK.md) |
| DEC-0033 | 2026-08-16 | v1.10 Phase 03 unified explanation + graph projection: `DaySnapshot.traditional_wellness: Option<TraditionalWellnessContext>` is the additive Option<T> container that collapses the `.1` hour-branch and `.2` seasonal-cultivation cores onto a single consumer surface (EXPLAIN-01). The context carries the branch primitive (`shi-er-jing-na-di-zhi`), the solar-term primitive (`amlich-solar-term-engine` Snapshot family), the Suwen primitive (`huangdi-neijing-suwen`), and exactly one composite (`rule.composite.seasonal_wellness`) envelope — never collapsed. Semantic-graph nodes `TraditionalChannel` + `SeasonalProfile` and edges `AssociatedWithHourBranch` + `JoinedByTermToSeason` are first-class ontology additions with locked concept-label round-trip; the desktop Almanac Inspector renders the bilingual explanation, disclaimer, review state, time basis, and divergence details in a new `classical-v110-wellness-surface` section. Day Assessment / Hour Ranking / Direction Assessment remain untouched (ADR-0003). | [../milestones/v1.10-phases/03-unified-explanation-and-graph-projection/03-01-PLAN.md](../milestones/v1.10-phases/03-unified-explanation-and-graph-projection/03-01-PLAN.md) |
