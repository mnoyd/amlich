use crate::semantic_graph::{
    EdgeConcept, NodeConcept, NodeOrigin, ProvenanceEntry,
    SemanticEdge, SemanticGraph, SemanticId, SemanticNode,
};
use crate::interaction::types::{
    BranchRelation, DirectionMergeMatrix, DirectionSignal, DomainDayBoostMatrix,
    ElementInteraction, ElementResonanceMatrix, PersonalHourMatrix, DayPersonMatrix,
};
...
impl InteractionGraphBuilder {
    ...
    pub fn add_matrix_root(
        ...
        let node_concept_label = matrix_kind.as_str().unwrap_or("matrix").replace("_matrix", "");
        ...
        .with_tags(vec![
            format!("matrix:{}/{}", node_concept_label, self.day_id),
        ])
    ...
    pub fn add_row_node(
        ...
        if let Some(s) = score {
            node = node.with_tags(vec![format!("score:{}", s)]);
        }
    ...
    pub fn add_ten_god_relation_node(
        ...
        let stable_key = format!("{}:{}:{}:{}", row_node_id, self.day_id, self.profile_id, pillar_key);
        let provenance = self.provenance(&stable_key, "ten_god_relation");

        let polarity = if is_favorable { "favorable" } else { "unfavorable" };
        let node = SemanticNode::new(
        ...
        .with_tags(vec![label.to_string(), polarity.to_string()])
        .with_provenance(provenance);

        self.graph.add_node(node);

        let edge = SemanticEdge::new(row_node_id, &stable_key, EdgeConcept::HasTenGodRelation);
        self.graph.add_edge(edge);
    }
    ...
    pub fn add_branch_relation_node(
        ...
        let edge = SemanticEdge::new(row_node_id, &stable_key, EdgeConcept::HasBranchRelation);
        self.graph.add_edge(edge);
    }
    ...
    pub fn add_element_relation_node(
        ...
        let (label, polarity) = match interaction {
            ...
            ElementInteraction::DayGeneratesPillar => ("day_generates_pillar".to_string(), "favorable".to_string()),
            ElementInteraction::PillarGeneratesDay => ("pillar_generates_day".to_string(), "mild_favorable".to_string()),
            ElementInteraction::DayControlsPillar => ("day_controls_pillar".to_string(), "challenging".to_string()),
            ElementInteraction::PillarControlsDay => ("pillar_controls_day".to_string(), "challenging".to_string()),
        };
        ...
        .with_tags(vec![label, polarity])
        .with_provenance(provenance);

        self.graph.add_node(node);

        let edge = SemanticEdge::new(row_node_id, &stable_key, EdgeConcept::HasElementRelation);
        self.graph.add_edge(edge);
    }
    ...
    pub fn add_direction_signal_node(
        ...
        let stable_key = format!("{}:{}:{}:{}", matrix_id, self.day_id, self.profile_id, direction);
        ...
        .with_tags(vec![signal_label.to_string(), polarity.to_string()])
        .with_provenance(provenance);

        self.graph.add_node(node);

        let edge = SemanticEdge::new(matrix_id, &stable_key, EdgeConcept::RelatesTo);
        self.graph.add_edge(edge);
    }
}
...
pub fn build_day_person_matrix_graph(
    ...
    let matrix_node_id = builder.add_matrix_root(
        &matrix_id,
        NodeConcept::DayPersonMatrix,
        &format!("Day-Person Matrix: {} vs {}", matrix.day_canchi, matrix.day_master),
    );
...
pub fn build_personal_hour_matrix_graph(
    ...
    let matrix_id = format!("matrix:{}:{}:{}", matrix_kind, day_id, profile_id);

    let mut builder = InteractionGraphBuilder::new(day_id, profile_id);
    let matrix_node_id = builder.add_matrix_root(
        &matrix_id,
        NodeConcept::PersonalHourMatrix,
        &format!("Personal Hour Matrix: {} for {}", matrix.day_canchi, matrix.day_master),
    );

    for hour in &matrix.hours {
        let slot_key = hour.chi_index.to_string();
        let row_id = format!("{}:{}:{}:{}", matrix_kind, day_id, profile_id, slot_key);
        let polarity = if hour.is_hoang_dao { "favorable" } else { "unfavorable" };
        let row_node_id = builder.add_row_node(
            &row_id,
...
pub fn build_element_resonance_matrix_graph(
    ...
    let mut builder = InteractionGraphBuilder::new(day_id, profile_id);
    builder.add_matrix_root(
        &matrix_id,
        NodeConcept::ElementResonanceMatrix,
        &format!("Element Resonance: {} in month {}", matrix.day_canchi, matrix.month_chi),
    );
    ...
    for entry in &matrix.entries {
        ...
        let _row_node_id = builder.add_row_node(
            &row_id,
            NodeConcept::InteractionRow,
            &format!("{} element: score={} resonance={:.2}", elem_key, entry.personal_score, entry.effective_resonance),
            None,
            Some(polarity),
        );
    }
...
pub fn build_direction_merge_matrix_graph(
    ...
    builder.add_matrix_root(
        ...
        &format!("Direction Merge: {} kua={}", matrix.day_canchi, matrix.kua_number),
    );

    for entry in &matrix.entries {
        let row_id = format!("{}:{}:{}:{}", matrix_kind, day_id, profile_id, entry.direction);
        ...
        builder.add_has_row_edge(&matrix_id, &row_node_id);

        for signal in &entry.signals {
            builder.add_direction_signal_node(&matrix_id, &entry.direction, *signal);
        }
    }
...
pub fn build_domain_day_boost_matrix_graph(
    ...
    for entry in &matrix.entries {
        ...
        let row_node_id = builder.add_row_node(
            &row_id,
            NodeConcept::InteractionRow,
            &format!("{} domain: {} → {}", entry.domain, entry.base_score, entry.boosted_score),
            None,
            Some(polarity),
        );
        builder.add_has_row_edge(&matrix_id, &row_node_id);
...
fn polarity_for_pillar<'a>(branch_rel: &BranchRelation, elem_interaction: &ElementInteraction) -> &'a str {
    ...
    match elem_interaction {
        ...
        ElementInteraction::DayControlsPillar | ElementInteraction::PillarControlsDay => "challenging",
...
fn thap_than_is_favorable(label: crate::almanac::types::ThapThanLabel) -> bool {
    ...
    matches!(label, ChinhAn | ThienAn | TyKien | KiepTai | ThucThan | ChinhQuan)
}
...
pub fn project_day_person_matrix(
    ...
    Err("projection not yet implemented".to_string())
}
...
mod tests {
    ...
    use crate::almanac::tu_menh::{KuaGroup, KuaResult, ConventionMetadata, Direction};
    use crate::almanac::types::{DayFortune, DayElement, DayConflict, TravelDirection, DayStars, DayDeity, DayDeityClassification, XungHopResult, TrucInfo};
    ...
    fn day_person_matrix_graph_has_ten_god_nodes() {
        ...
        assert!(graph.node_count() > 5, "should have root + 4 rows + relation nodes");
    }
    ...
    fn personal_hour_matrix_graph_has_12_rows() {
        ...
        let graph = build_personal_hour_matrix_graph("2024-01-01", "test-profile", &matrix).unwrap();

        for slot in 0..12 {
            let row_id = format!("row:personal_hour:2024-01-01:test-profile:{}", slot);
            assert!(graph.has_node(&row_id), "should have hour slot row: {}", row_id);
        }
    }
    ...
    fn element_resonance_matrix_graph_has_5_rows() {
        ...
        let graph = build_element_resonance_matrix_graph("2024-01-01", "test-profile", &matrix).unwrap();

        let elements = vec!["moc", "hoa", "tho", "kim", "thuy"];
        for elem in elements {
            let row_id = format!("row:element_resonance:2024-01-01:test-profile:{}", elem);
            assert!(graph.has_node(&row_id), "should have element row: {}", row_id);
        }
    }
    ...
    fn direction_merge_matrix_graph_has_8_rows() {
        ...
        let graph = build_direction_merge_matrix_graph("2024-01-01", "test-profile", &matrix).unwrap();

        let directions = vec!["Bắc", "Đông Bắc", "Đông", "Đông Nam", "Nam", "Tây Nam", "Tây", "Tây Bắc"];
        for dir in directions {
            let row_id = format!("row:direction_merge:2024-01-01:test-profile:{}", dir);
            assert!(graph.has_node(&row_id), "should have direction row: {}", row_id);
        }
    }
    ...
    fn domain_day_boost_matrix_graph_has_5_domains() {
        ...
        let graph = build_domain_day_boost_matrix_graph("2024-01-01", "test-profile", &matrix).unwrap();
        ...
        for domain in domains {
            ...
            assert!(graph.has_node(&row_id), "should have domain row: {}", row_id);
        }
    }
    ...
    fn all_matrix_graphs_build_successfully() {
        ...
        assert!(dpm_graph.node_count() > 4, "day person matrix graph should have root + rows + relations");
        assert!(dpm_graph.edge_count() > 0, "day person matrix graph should have edges");
        ...
        assert!(phm_graph.node_count() >= 13, "personal hour matrix graph should have root + 12 hour rows");
        assert!(phm_graph.edge_count() > 0, "personal hour matrix graph should have edges");
        ...
        assert!(erm_graph.node_count() >= 6, "element resonance graph should have root + 5 element rows");
        ...
        assert!(dmm_graph.node_count() >= 9, "direction merge graph should have root + 8 direction rows");
        ...
        assert!(ddb_graph.node_count() >= 6, "domain day boost graph should have root + 5 domain rows");
    }
}
