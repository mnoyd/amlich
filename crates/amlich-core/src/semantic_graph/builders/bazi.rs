...
impl BaziGraphBuilder {
    ...
    fn add_profile_root(&mut self, chart: &BaziChart) {
        ...
        let label = format!("Bazi {} tuổi, giờ {}", chart.input.year, chart.input.hour);
    ...
    fn add_year_pillar(&mut self, chart: &BaziChart) {
        ...
        let node_id = SemanticId::pillar_bazi(
            "year",
            &format!(
                "{:04}-{:02}-{:02}T{:02}:{:02}",
                chart.input.year,
        ...
        .with_tags(vec![pillar.can_chi.can.clone(), pillar.can_chi.chi.clone()])
        .with_provenance(provenance);
    ...
    fn add_day_master(&mut self, chart: &BaziChart) {
        ...
        let node_id = SemanticId::new("day_master", format!("bazi:{}:{}", self.tz_suffix, "self"))
            .to_node_id();
    ...
    fn add_element_distribution(&mut self, chart: &BaziChart, analysis: &BaziAnalysisReport) {
        ...
        let node_id = SemanticId::element_distribution_bazi(
            &format!(
                "{:04}-{:02}-{:02}T{:02}:{:02}",
                chart.input.year,
                chart.input.month,
    ...
    fn add_ten_god_distribution(&mut self, chart: &BaziChart, analysis: &BaziAnalysisReport) {
        ...
        let node_id = SemanticId::ten_god_distribution_bazi(
            &format!(
                "{:04}-{:02}-{:02}T{:02}:{:02}",
                chart.input.year,
                chart.input.month,
    ...
    fn add_day_master_strength(&mut self, chart: &BaziChart, analysis: &BaziAnalysisReport) {
        ...
        let node_id = SemanticId::day_master_strength_bazi(
            &format!(
                "{:04}-{:02}-{:02}T{:02}:{:02}",
                chart.input.year,
                chart.input.month,
        ...
        let node = SemanticNode::new(
            SemanticId::new(
                "day_master_strength",
                format!("bazi:{}:dms", self.tz_suffix),
            ),
            NodeConcept::PersonalAlignment,
    ...
    fn add_chart_interactions(&mut self, chart: &BaziChart, analysis: &BaziAnalysisReport) {
        for (i, interaction) in analysis.interactions.iter().enumerate() {
            let node_id = SemanticId::chart_interaction_bazi(
                &format!(
                    "{:04}-{:02}-{:02}T{:02}:{:02}",
                    chart.input.year,
                    chart.input.month,
...
pub fn build_bazi_profile_graph(chart: &BaziChart, analysis: &BaziAnalysisReport) -> SemanticGraph {
...
mod tests {
    ...
    fn bazi_graph_contains_profile_root() {
        ...
        assert!(
            ...
            "profile root should exist"
        );
        assert!(
            graph.node_count() >= 5,
            "should have profile + pillars + analysis nodes"
        );
    }
    ...
    fn bazi_graph_contains_pillars() {
        ...
        assert!(
            pillar_ids.len() >= 3,
            "should have year, month, day pillars minimum, got: {:?}",
            pillar_ids
        );
    }
    ...
    fn bazi_graph_contains_analysis_nodes() {
        ...
        let has_dms = graph
            .nodes()
            .keys()
            .any(|id| id.contains("day_master_strength"));
    ...
    fn bazi_graph_different_inputs_different_ids() {
        ...
        assert_ne!(
            ids1, ids2,
            "different inputs should produce different node IDs"
        );
    }
    ...
    fn bazi_graph_stable_profile_root_id() {
        ...
        assert!(
            graph.has_node(&expected_profile_id),
            "profile root should be at expected stable ID: {}",
            expected_profile_id
        );
    }
    ...
    fn bazi_graph_edge_kind_is_composes() {
        ...
        for edge in graph.outgoing_edges(&profile_root_id) {
            assert_eq!(
                ...
                "edges from profile root should be Composes"
            );
        }
    }
    ...
    fn bazi_graph_contains_expected_fact_kinds() {
        ...
        let has_chart_pillar = graph
            .nodes()
            .values()
            .any(|n| matches!(n.concept, NodeConcept::ChartPillar));
        let has_element = graph
            .nodes()
            .values()
            .any(|n| matches!(n.concept, NodeConcept::Element));
        let has_personal_alignment = graph
            .nodes()
            .values()
            .any(|n| matches!(n.concept, NodeConcept::PersonalAlignment));
        ...
        assert!(
            ...
            "should have personal alignment (profile/day master strength) nodes"
        );
    }
    ...
    fn bazi_graph_determinism_across_multiple_builds() {
        ...
        assert_eq!(
            ids1, ids2,
            "same inputs should produce identical node IDs across builds"
        );
    }
}
