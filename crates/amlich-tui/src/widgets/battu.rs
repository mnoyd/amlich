...
impl Widget for BattuWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let day_str = format!(
            "Ngày   {:<12} {}",
            canchi.day.full,
            format_nguhanh(&canchi.day.ngu_hanh.can, &canchi.day.ngu_hanh.chi)
        );
        let month_str = format!(
            "Tháng  {:<12} {}",
            canchi.month.full,
            format_nguhanh(&canchi.month.ngu_hanh.can, &canchi.month.ngu_hanh.chi)
        );
        let year_str = format!(
            "Năm    {:<12} {} {}",
            canchi.year.full,
            format_nguhanh(&canchi.year.ngu_hanh.can, &canchi.year.ngu_hanh.chi),
            get_animal_emoji(&canchi.year.chi)
        );
