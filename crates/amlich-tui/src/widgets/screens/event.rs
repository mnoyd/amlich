...
impl Widget for EventScreenWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(event_lines(self.app))
            .wrap(Wrap { trim: true })
            .render(inner, buf);
    }
}

pub fn event_natural_height(app: &AppState, area_width: u16) -> u16 {
    let block_height = 2usize;
    let inner_width = usize::from(area_width.saturating_sub(2).max(1));
    let content_height = event_lines(app)
        .iter()
...
pub fn event_lines(app: &AppState) -> Vec<Line<'static>> {
    ...
    let mut lines = vec![];
    let compact = matches!(app.active_verbosity(), VerbosityMode::Compact);
...
fn wrapped_line_height(line: &Line<'_>, inner_width: usize) -> usize {
    line.width().max(1).div_ceil(inner_width.max(1))
}
